<x-layouts.app :title="'Edit Meeting — Zoom Scheduler'">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-8">
            ...
            <p class="text-white/40 text-sm mt-1">
                Ubah detail meeting Zoom Anda
            </p>
        </div>

        @php
            $mStartDate = $meeting->start_time ? \Carbon\Carbon::parse($meeting->start_time)->setTimezone($meeting->timezone)->format('Y-m-d') : now()->format('Y-m-d');
            $mStartTime = $meeting->start_time ? \Carbon\Carbon::parse($meeting->start_time)->setTimezone($meeting->timezone)->format('H:i') : now()->format('H:i');
        @endphp

        {{-- Meeting Form --}}
        <div class="glass-card rounded-2xl p-6 sm:p-8 animate-fade-in-up">
            <form method="POST" action="{{ route('meetings.update', $meeting) }}" id="meeting-form">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div class="relative" id="custom-select-container">
                        ...
                        <input type="hidden" id="zoom_account_id" name="zoom_account_id" value="{{ old('zoom_account_id', $meeting->zoom_account_id) }}" required>
                        ...
                        <button type="button" onclick="toggleDropdown()" class="input-field text-left flex items-center justify-between cursor-pointer" id="dropdown-trigger">
                            <span id="dropdown-selected-text" class="text-white">Pilih Akun Zoom...</span>
                    ...
                    <div>
                        ...
                        <textarea id="agenda" name="agenda" rows="3"
                            ...
                            class="input-field">{{ old('agenda', $meeting->agenda) }}</textarea>
                        @error('agenda')
                            <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <label class="cursor-pointer relative flex flex-col p-4 rounded-xl border bg-white/[0.01] hover:bg-white/[0.03] transition-all duration-200 select-none" id="label-type-instant">
                                <input type="radio" name="type" value="1" class="sr-only" {{ old('type', $meeting->type) == '1' ? 'checked' : '' }} onchange="toggleType(1)">
                            ...
                            <label class="cursor-pointer relative flex flex-col p-4 rounded-xl border bg-white/[0.01] hover:bg-white/[0.03] transition-all duration-200 select-none" id="label-type-scheduled">
                                <input type="radio" name="type" value="2" class="sr-only" {{ old('type', $meeting->type) == '2' ? 'checked' : '' }} onchange="toggleType(2)">
                                <div class="flex items-start sm:items-center gap-3 mb-1">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" id="start_date" name="start_date"
                                    value="{{ old('start_date', $mStartDate) }}"
                            ...
                            <div>
                                ...
                                <input type="text" id="start_time" name="start_time"
                                    value="{{ old('start_time', $mStartTime) }}"
                                    class="input-field cursor-pointer bg-white/[0.02]">
                                @error('start_time')
                                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                                @enderror
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select id="duration" name="duration" class="input-field cursor-pointer">
                                    @php $dur = old('duration', $meeting->duration); @endphp
                                    <option value="15" {{ $dur == '15' ? 'selected' : '' }}>15 menit</option>
                                    <option value="30" {{ $dur == '30' ? 'selected' : '' }}>30 menit</option>
                                    <option value="45" {{ $dur == '45' ? 'selected' : '' }}>45 menit</option>
                                    <option value="60" {{ $dur == '60' ? 'selected' : '' }}>1 jam (60 mnt)</option>
                                    <option value="90" {{ $dur == '90' ? 'selected' : '' }}>1.5 jam (90 mnt)</option>
                                    <option value="120" {{ $dur == '120' ? 'selected' : '' }}>2 jam (120 mnt)</option>
                                    <option value="180" {{ $dur == '180' ? 'selected' : '' }}>3 jam (180 mnt)</option>
                                    <option value="240" {{ $dur == '240' ? 'selected' : '' }}>4 jam (240 mnt)</option>
                                    <option value="300" {{ $dur == '300' ? 'selected' : '' }}>5 jam (300 mnt)</option>
                                    <option value="360" {{ $dur == '360' ? 'selected' : '' }}>6 jam (360 mnt)</option>
                                    <option value="480" {{ $dur == '480' ? 'selected' : '' }}>8 jam (480 mnt)</option>
                                    <option value="600" {{ $dur == '600' ? 'selected' : '' }}>10 jam (600 mnt)</option>
                                    <option value="720" {{ $dur == '720' ? 'selected' : '' }}>12 jam (720 mnt)</option>
                                    <option value="1440" {{ $dur == '1440' ? 'selected' : '' }}>24 jam (1440 mnt)</option>
                                </select>
                                @error('duration')
                                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <select id="timezone" name="timezone" class="input-field cursor-pointer">
                                    @php $tz = old('timezone', $meeting->timezone); @endphp
                                    <option value="Asia/Jakarta" {{ $tz == 'Asia/Jakarta' ? 'selected' : '' }}>WIB - Jakarta (GMT+7)</option>
                                    <option value="Asia/Makassar" {{ $tz == 'Asia/Makassar' ? 'selected' : '' }}>WITA - Makassar (GMT+8)</option>
                                    <option value="Asia/Jayapura" {{ $tz == 'Asia/Jayapura' ? 'selected' : '' }}>WIT - Jayapura (GMT+9)</option>
                                    <option value="UTC" {{ $tz == 'UTC' ? 'selected' : '' }}>UTC (GMT+0)</option>
                                </select>
                                @error('timezone')
                                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                                @enderror
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const oldId = "{{ old('zoom_account_id', $meeting->zoom_account_id) }}";
            if (oldId) {
                if (oldId === 'auto') {
    ...
    <script>
        ...
        flatpickr("#start_time", {
            ...
            dateFormat: "H:i",
            time_24hr: true,
            defaultDate: "{{ old('start_time', $mStartTime) }}",
            minuteIncrement: 5,
            disableMobile: true,
        });
    </script>
